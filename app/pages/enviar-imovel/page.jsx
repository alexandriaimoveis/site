        ...
        <div className="w-full max-w-2xl mx-auto bg-white border border-gray-300 rounded-3xl shadow-sm p-8">
          {sucesso ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button disabled={loading} className="w-full h-12 bg-[#F29829] hover:bg-[#1F3445] text-white font-bold rounded-md transition duration-300">
...
function Input({ label, value, onChange, type = "text" }) {
  return (
    <div className="flex flex-col gap-1">
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-[#F29829]" />
    </div>
  );
}